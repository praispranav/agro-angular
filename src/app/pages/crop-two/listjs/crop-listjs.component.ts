...
import {
  ...
  SortEvent,
} from "./crop-two-listjs-sortable.directive";

@Component({
  selector: "app-crop-two-listjs",
...
export class CropListjsComponent {
  ...
  getVerityType(): void {
    this.service.verityTypeList().subscribe(
      (res: any) => {
        this.verityTypeList = res.data;
      },
      (err) => {
        console.log("err", err);
      }
  ...
  openVerityTypePopup(verityType: any): any {
    this.submitted = false;
    this.modalService.open(verityType, { size: "lg", centered: true });
  }
